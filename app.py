...
price_model = tf.keras.models.load_model('./models/best_laptop_price_model.keras')
review_model = tf.keras.models.load_model('./models/best_laptop_review_model.keras')

# Load your laptop data
laptop = pd.read_csv("./data/predicted_laptops.csv")
...
def recommend(request: LaptopRequest):
    try:
        ...
        if not filtered_laptops.empty:
            # Ensure the encoded columns exist
            if all(col in filtered_laptops.columns for col in ['processor_encoded', 'ram_encoded', 'storage_encoded']):
                ...
                raise HTTPException(status_code=500, detail="Encoded columns are missing in the laptop data.")
        else:
